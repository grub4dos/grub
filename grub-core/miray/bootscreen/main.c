		...
		int argc, char *argv[])
{
   ...
   if (argc > 0)
   {
      ...
      int i;
      const char ** tmp = grub_calloc((argc + 1), sizeof(char *));
      for (i = 0; i < argc; i++)
         tmp[i] = argv[i];
...
static grub_err_t
miray_cmd_warning(struct grub_command *cmd __attribute__ ((unused)),
		int argc, char *argv[])
{
   if (argc == 0)
      miray_screen_warning(NULL);
   else
      miray_screen_warning(argv[0]);
...
static grub_command_t cmd_center;
static grub_command_t cmd_submenu;
static grub_command_t cmd_fatal;
static grub_command_t cmd_warning;
...
GRUB_MOD_INIT(miray_bootscreen)
{
   ...
   cmd_warning = grub_register_command("miray_warning",
                                        miray_cmd_warning,
                                        0, N_("display a warning message"));
...
GRUB_MOD_FINI(miray_bootscreen)
{
   ...
   grub_unregister_command (cmd_warning);
