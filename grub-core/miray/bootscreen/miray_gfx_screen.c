...
static void miray_gfx_screen_draw_rect_single (struct miray_screen * scr, const grub_video_rect_t * bounds, int clear)
{
   ...
   switch(data->fg_mode)
   {
      ...
      case FG_DEFAULT:
         if (timeout > 0)
         {
            if (screen_data.timeout_format != 0 && grub_printf_fmt_check(screen_data.timeout_format, "%d") == GRUB_ERR_NONE && data->label_timeout2 != 0)
            {
...
static void miray_gfx_screen_finish (struct miray_screen * scr)
{
   ...
   {
      // Add a small 4 x 4 area in the top left corner to allow the os to check if the fb pitch values are correct
      int i;
      grub_video_rgba_color_t col;
      
      col.alpha = 255;
      for (i = 0; i < 4; i++)
      {
         col.red = col.green = col.blue = i + 1;
         grub_video_fill_rect (grub_video_map_rgba_color (col),
                            i, 0, 1, 4);
      }
